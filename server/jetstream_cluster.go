...
func (cc *jetStreamCluster) nodeForConsumerProposals(sa *streamAssignment) RaftNode {
	if sa.Config.ManagesConsumers {
		if sa.Group == nil || sa.Group.node == nil || !sa.Group.node.Leader() {
			return nil
		}
...
func (js *jetStream) monitorStream(mset *stream, sa *streamAssignment, sendSnapshot bool) {
	...
	const (
		compactInterval = 15 * time.Second // 2 * time.Minute
	...
	doSnapshot := func() {
		...
		ne, nb := n.Size()
		if len(mset.consumers) == 0 && curState == lastState && ne < compactNumMin && nb < compactSizeMin {
			return
		}
		...
		case err := <-restoreDoneCh:
			...
			if consumers := mset.getPublicConsumers(); len(consumers) > 0 {
				for _, o := range consumers {
					...
					cc.nodeForConsumerProposals(sa).ForwardProposal(addEntry)
	...
	for {
		...
		case <-mmtc:
			...
			if newPeerSet[ourPeerId] {
				...
				js.cluster.nodeForConsumerProposals(o.mset.sa).ForwardProposal(encodeAddConsumerAssignment(cca))
...
func (s *Server) jsClusteredStreamUpdateRequest(ci *ClientInfo, acc *Account, subject, reply string, rmsg []byte, cfg *StreamConfig, peerSet []string, pedantic bool) {
	...
	node := cc.nodeForConsumerProposals(sa)
	for _, ca := range consumers {
		node.Propose(encodeAddConsumerAssignment(ca))
	}
}
