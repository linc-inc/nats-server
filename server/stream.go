	...
	if isLeader {
		...
		if mset.cfg.ManagesConsumers {
			...
			pairs := []struct {
				...
				{fmt.Sprintf(JSApiConsumerCreateExT, mset.cfg.Name, "*", "*"), mset.srv.jsConsumerCreateRequest},
			...
			for _, p := range pairs {
				...
				if err := mset.js.apiSubs.Insert(sub); err != nil {
					return err
				}
			}
		}
